    ...
    <v-card>
      ...
      <v-dialog v-model="dialog" persistent width="800">
        <v-card
          ...
          :title="$t('frontend.contents.busStops.title')"
          :subtitle="editForm.busStopName"
        >
          <v-card-text>
            <v-form validate-on="eager" @update:model-value="dialogValidate">
              <!-- Edit Form Start -->
              <v-text-field
                v-model="editForm.busStopName"
              ...
              <v-text-field
                v-model="editForm.busStopLocation"
                :rules="[$rules.requried]"
                label="Location"
...
export default {
  data: () => ({
    ...
    config: {
      ...
      searchOptions: [
        ...
        { value: 'busStopName', text: '정류장' },
        { value: 'busStopLocation', text: '정류장위치' }
      ],
      ...
      headers: [
        ...
        { key: "busStopName", title: "Name", align: "center" },
        { key: "busStopLocation", title: "Location", align: "end" },
      ],
      sortBy: [
        { key: "id", order: "desc" }
      ],
  ...
  methods: {
    ...
    // handle
    handleCreate(){
      return $busServer.busStops.create(this.editForm);
    },
    handleRead(entity){
      return $busServer.busStops.read(entity);
    },
    handleUpdate(){
      return $busServer.busStops.update(this.editForm);
    },
    handleDelete(){
      return $busServer.busStops.delete(this.editForm);
    },
    handleSearch(query){
      return $busServer.busStops.search(this.searchForm, query);
    },
    handleEntities(res){
      this.entitiesTotal = res.page.totalElements;
      this.entities = res._embedded.busStops;
      return res;
    },
    handleEntity(res){
